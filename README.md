...
- **Honest satellites.** SGP4 propagation with orbit rings that stay locked to their satellites via GMST realignment — no drift, no per-second flicker. Fleet propagation runs off the main thread and falls back to the same math when a worker is unavailable.
