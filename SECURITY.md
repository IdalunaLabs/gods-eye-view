...
- **App-level throttles:** `GEV_RATELIMIT_OPENAI_PER_MIN` and `GEV_RATELIMIT_GOOGLE_PER_MIN` cap the cost-bearing endpoints per client IP per minute when set (over-limit requests receive a sanitized `429`). `GEV_RATELIMIT_GBFS_PER_MIN` caps `/api/gbfs` and defaults to 120/min (`0` disables). `GEV_RATELIMIT_ADSBDB_PER_MIN` caps `/api/adsbdb` and defaults to 60/min (`0` disables). `GEV_RATELIMIT_TERRAIN_PER_MIN` caps `/api/terrain/heights` and defaults to 120/min (`0` disables). They are **per-IP, process-local, in-memory guards** — they reset on restart and are **not billing caps**.
