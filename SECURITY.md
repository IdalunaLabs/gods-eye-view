...
- **No arbitrary-URL fetching.** The CCTV frame proxy fetches only server-registered camera/frame URLs — clients cannot pass an upstream URL to fetch (SSRF mitigation). Other proxies target fixed upstream hosts. Registered CCTV URLs are checked again at fetch time: only `http:`/`https:` with no credentials, and DNS answers (including IPv6 and IPv4-mapped forms) must be public. Loopback, private, link-local, and multicast targets are rejected. Media bodies are aborted when they pass 64 MiB or go idle for 15 seconds; they are not buffered whole. Street View fallback uses the registered camera's coordinates only and shares `GEV_RATELIMIT_GOOGLE_PER_MIN`.
- **GBFS is allowlisted and metered.** `/api/gbfs` accepts only HTTPS `station_information` and `station_status` URLs on the operator allowlist, refuses redirects, and caps streamed bodies at 5 MiB. Successful `station_information` feeds are stored in a 48-entry LRU (live `station_status` is not cached). A per-IP limit of 120/min is on unless `GEV_RATELIMIT_GBFS_PER_MIN=0`.
- **adsbdb keys and caches are bounded.** `/api/adsbdb` accepts a callsign (`[A-Z0-9]{1,8}`) or ICAO hex (`[0-9a-f]{6}`) before lookup or cache write, and keeps registrations to `[A-Z0-9-]{1,12}`. Each store is an LRU of 2048 entries; the flushed disk cache is also byte-capped. A per-IP limit of 60/min is on unless `GEV_RATELIMIT_ADSBDB_PER_MIN=0`.
- **Terrain heights are bounded.** `/api/terrain/heights` accepts finite latitude `[-90, 90]` and longitude `[-180, 180]`, rejects batches over 2000 points with a sanitized 400, keeps at most 20,000 points in memory, and evicts on flush so the disk cache stays within 8 MiB. A per-IP limit of 120/min is on unless `GEV_RATELIMIT_TERRAIN_PER_MIN=0`. The browser height cache is a 10,000-entry LRU.
...
- **App-level throttles:** `GEV_RATELIMIT_OPENAI_PER_MIN` caps `/api/realtime/token` and `/api/openai/hud-summary`. It is unlimited on a loopback bind when unset, and **10/min/IP when `HOST` is not loopback and the variable is unset** (`0` disables). `GEV_RATELIMIT_GOOGLE_PER_MIN` stays opt-in. `GEV_RATELIMIT_GBFS_PER_MIN` caps `/api/gbfs` and defaults to 120/min (`0` disables). `GEV_RATELIMIT_ADSBDB_PER_MIN` caps `/api/adsbdb` and defaults to 60/min (`0` disables). `GEV_RATELIMIT_TERRAIN_PER_MIN` caps `/api/terrain/heights` and defaults to 120/min (`0` disables). They are **per-IP, process-local, in-memory guards** — they reset on restart and are **not billing caps**.
- **No shared API access token in the client.** `GEV_API_ACCESS_TOKEN` is not enforced. The Realtime client would have to send it, which puts the secret in the browser. A non-loopback deployment needs an authenticating reverse proxy in front of this server; do not add the token to the Vite `define` block.
